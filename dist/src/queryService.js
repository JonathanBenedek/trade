...
exports.getQuery = exports._averageDiffrences = exports._findLastLowOperation = exports._averageOperation = exports._maxOperation = exports._minOperation = void 0;
const httpsService_1 = require("./httpsService");
...
function _getOperation(data, location) {
...
function _extractPositionRange(range) {
    const splitedRanges = range.split('-');
...
function _minOperation(data, location) {
    ...
    let minValue = 999999;
    let hasMin = false;
    let { start, end } = _extractPositionRange(position);
    for (const [param, value] of Object.entries(data)) {
        if (start > index) {
            index = index + 1;
            continue;
...
exports._minOperation = _minOperation;
function _maxOperation(data, location) {
...
exports._maxOperation = _maxOperation;
function _averageOperation(data, location) {
...
exports._averageOperation = _averageOperation;
function _findLastLowOperation(data, location) {
...
exports._findLastLowOperation = _findLastLowOperation;
...
function _averageDiffrences(data, location) {
...
exports._averageDiffrences = _averageDiffrences;
function extractByOperation(data, operation, location) {
    switch (operation) {
        case "get":
...
function getQuery(query, symbol) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let response = yield httpsService_1.httpsService.get(request, null);
        if (response.Note) {
            //console.log(`response.Note = ${response.Note}`)
            yield awaitOneMinIfNeeded();
            response = yield httpsService_1.httpsService.get(request, null);
        }
        if (response.Note) {
            console.log(`response.Note = ${response.Note}`);
            awaitOneMinIfNeeded();
